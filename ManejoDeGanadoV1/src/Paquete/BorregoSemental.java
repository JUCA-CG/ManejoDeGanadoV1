...
public class BorregoSemental extends JFrame implements ActionListener {
librer cc= new librer(); 
Connection cn = cc.conexion();

    // Componentes de la ventana
    private JPanel panel;
    private JLabel labelArete, labelPeso,labelcantidad, labelEdad, labelRaza, labelColor, labelObservaciones, labelTitulo;
   ...
   private DefaultTableModel modelo;
    ...
    public BorregoSemental() {
        // Configuración de la ventana
        setTitle("Registro de Borrego Semental");
        setSize(1000, 650);
       ...
       // Configuración del panel principal
        ...
        //Etiqueta de título
        labelTitulo = new JLabel("Borregos Sementales");
        labelTitulo.setBounds(270, 30, 200, 25);
    ...
    panel.add(textAreaObservaciones);
     ...
     // Etiqueta de De cantidad de borregos Sementales totales
    int totalBorregas = obtenerTotalBorregosS();
    labelcantidad = new JLabel("Borregos sementales totales: " + totalBorregas);
...
/// Tabla
   ...
   // Barra de menú
    ...
    // Opción de borrego semental (seleccionada)
    menuItemBorregoSemental = new JMenuItem("Borrego Semental");
    menuItemBorregoSemental.setEnabled(false);
    menuOpciones.add(menuItemBorregoSemental);

    // Opción de borrega cria
...
void mostrarDatos(){
    ...
    String sql = "SELECT * FROM borregosSementales WHERE numero_arete='" + valor + "'";
    ...
    try{
        ...
        Logger.getLogger(BorregoSemental.class.getName()).log(Level.SEVERE, null, ex);
...
private void mostrarTodo() {
    try {
        ...
        String sql = "SELECT * FROM borregosSementales";
...
public void actionPerformed(ActionEvent e) {
    if (e.getSource() == botonAgregar) {
        // Código para agregar un borrego
...
try {
pps = cn.prepareStatement("INSERT INTO borregosSementales(numero_arete,color,raza,peso,edad,observaciones)VALUES (?,?,?,?,?,?)");
...
JOptionPane.showMessageDialog(null, "Datos guardados con éxito");
 int totalBorregos = obtenerTotalBorregosS();
  labelcantidad.setText("Borregos sementales totales: " + totalBorregos);
...
String valor= textFieldArete.getText();
try {
PreparedStatement pps = cn.prepareStatement("DELETE FROM borregosSementales WHERE numero_arete='"+valor+ "'");
pps.executeUpdate();
...
try {
PreparedStatement pps = cn.prepareStatement("UPDATE borregosSementales SET numero_arete = '"+valor+ "' ,color = '"+c+ "' ,raza = '"+r+ "' ,peso = '"+p+ "' ,edad = '"+ed+ "' ,observaciones = '"+o+ "'WHERE numero_arete='"+valor+ "'");
pps.executeUpdate();
...
//Boton VerTodo
   ...
   RegistroMedicoBorregos registroMedicoBorregos = new RegistroMedicoBorregos();
        registroMedicoBorregos.setVisible(true);
...
public int obtenerTotalBorregosS() {
    String consulta = "SELECT COUNT(*) FROM borregosSementales";
    Statement st = null;
    ResultSet rs = null;
    int totalBorregos = 0;
